...
export default function (pi: ExtensionAPI) {
	...
	return {
		...
		when: () => {
			if (readsSinceOutput < 50) return false;
			lastFiredCount = readsSinceOutput;
			readsSinceOutput = 0;
			return true;
		},
		message: () =>
			`You have done ${lastFiredCount} sequential reads without producing an edit, write, answer, or subagent launch. Pick ONE of these three, then act:\n\n` +
